...
while k < kt:
    s_sat, theta, epsi, taug, tauw, film,psi,chb=calc_Kaff_soil(pct_clay[0], pct_sand)
    factw=s_sat**(1./df)
    ...
    k2=k2*k2f
    Vmax=BT*k2
    dammf=damm_flx(O2,S[k],factw,Kaff_s*1.e0,Kaff_o2g,kappa_tops,Vmax)
    damfmax=np.max(dammf)
    dammf=dammf/damfmax
    #su model
    suf=su_flx(O2,S[k], factw,BT, k2*1.e0, k1_o2, k1_s,kappa_tops)
    sufmax=np.max(suf)
    suf=suf/sufmax
    #supeca model
    Kaff_s=k2/k1_s
    ms=0.0
    Km_s=2.0
    supf=supmic_flx(O2,S[k],factw, BT, ms, Km_s, Kaff_o2g, Kaff_s, kappa_tops, k2)
    supfmax=np.max(supf)
    supf=supf/supfmax
...
if plt_to_file:
    pdf=PdfPages('figure/FigureS2.pdf')
    fig=plt.figure()
font = {'family': 'serif',
        'color':  'black',
        'weight': 'normal',
...
plt.text(0.05, 0.9, '(a) DAMM',fontdict=font)
...
plt.subplot(223)
for n in range(4):
    plt.plot(s_sat,np.transpose(supfls[n][:]),label=r"S=%.1f mol m$^{-3}$"%(S[n],))
plt.yticks(np.arange(0, 1.2, step=0.25),[])
plt.yticks(np.arange(0, 1.2, step=0.25),[0.0,'',0.5,'',1.0])
...
plt.subplots_adjust(top=0.9, bottom=0.12, left=0.10, right=0.95, hspace=0.1,
                    wspace=0.15)
plt.text(0.125, 0.915, 'Substrate amount effect on moisture-respiration relationship',fontdict=font,transform=plt.gcf().transFigure)
