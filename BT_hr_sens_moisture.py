...
while k < kt:
    ...
    dammf=dammf/damfmax
    #su model
    suf=su_flx(O2,S, factw,BT[k], k2*1.e0, k1_o2, k1_s,kappa_tops)
    sufmax=np.max(suf)
    suf=suf/sufmax
    #supeca model
    ms=0.0
    Km_s=2.0
    supf=supmic_flx(O2,S,factw, BT[k], ms, Km_s, Kaff_o2g, Kaff_s, kappa_tops, k2)
    supfmax=np.max(supf)
    supf=supf/supfmax
...
plt.subplot(223)
for n in range(4):
    plt.plot(s_sat,np.transpose(supfls[n][:]),label=r"BT=%.1f C mol m$^{-3}$"%(mb[n],))
plt.yticks(np.arange(0, 1.2, step=0.25),[])
plt.yticks(np.arange(0, 1.2, step=0.25),[0.0,'',0.5,'',1.0])
...
plt.subplots_adjust(top=0.9, bottom=0.12, left=0.10, right=0.95, hspace=0.1,
                    wspace=0.15)
plt.text(0.125, 0.915, 'Microbial biomass effect on moisture-respiration relationship',fontdict=font,transform=plt.gcf().transFigure)
