...
if plt_to_file:
    pdf=PdfPages('figure/o2_varkaff.pdf')
    fig=plt.figure()
...
if plt_to_file:
    ...
    pdf=PdfPages('figure/o2full_varkaff.pdf')
    fig=plt.figure()
else:
    plt.show()
plt.subplot(221)
...
plt.subplot(221)
for n in range(4):
    plt.semilogy(s_sat,np.transpose(Kaff_o2g_full[n][:]),label="%d%s"%(pct_clay[n],'%',))
ymin, ymax = plt.ylim()
leg = plt.legend(loc='best', ncol=1)
leg.get_frame().set_alpha(0.)
...
plt.text(0.3, 0.95, r'Topsoil O$_2$ affinity (mol m$^{-3}$)',fontdict=font,transform=plt.gcf().transFigure)
plt.subplot(222)
#compute soil parameters
Kaff_o2g_full=[]
Kaff_o2g=[]
for om in fom:
    ...
    Kaff_o2g.append(Kaff_o2gi)
for n in range(4):
    plt.semilogy(s_sat,np.transpose(Kaff_o2g_full[n][:]),label="%d%s"%(fom[n]*100,'%',))
...
for n in range(4):
    plt.semilogy(s_sat,np.transpose(Kaff_o2g_full[n][:]),label="%d"%(Ncell[n],))
...
plt.text(0.35, 0.6, '(a) clay',fontdict=font,transform=plt.gcf().transFigure)
plt.text(0.825, 0.6, '(b) FOM',fontdict=font,transform=plt.gcf().transFigure)
plt.text(0.35, 0.15, r'(c) N$_{cell}$',fontdict=font,transform=plt.gcf().transFigure)
...
if plt_to_file:
    ...
    pdf=PdfPages('figure/ch4_varkaff.pdf')
    fig=plt.figure()
else:
    plt.show()
#===============================================================================
...
if plt_to_file:
    ...
    pdf=PdfPages('figure/solute_varkaff.pdf')
    fig=plt.figure()
else:
    plt.show()
