...
pct_sand=20.
pct_clay=[40.]
fom=[0.01,0.1,0.2,0.5]
kt=len(fom)
k=0

while k < kt:
    s_sat, theta, epsi, taug, tauw, film,psi,chb=calc_Kaff_soil(pct_clay[0], pct_sand,fom[k])
    factw=s_sat**(1./df)
...
if plt_to_file:
    pdf=PdfPages('figure/fom_hrsens.pdf')
    fig=plt.figure()
font = {'family': 'serif',
        'color':  'black',
        'weight': 'normal',
...
plt.subplot(223)
print np.shape(supfls)
for n in range(4):
    plt.plot(s_sat,np.transpose(supfls[n][:]),label="fom=%d%s"%(fom[n]*100,'%',))
plt.yticks(np.arange(0, 1.2, step=0.25),[])
plt.yticks(np.arange(0, 1.2, step=0.25),[0.0,'',0.5,'',1.0])
